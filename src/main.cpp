...
unsigned int nWorkTargetSpacing = 100;		// 100 sec block spacing for PoW
unsigned int nStakeTargetSpacing = 100;		// 100 sec block spacing for PoS
...
unsigned int nModifierInterval = 8 * 60; // Alexandrite - time to elapse before new modifier is computed
...
const string strMessageMagic = "Alexandrite Signed Message:\n";
...
const float c00 = 0.0016094379;
float computeFactor(int m)
{
	float d = std::exp(-c00 * (float)m);
	int nd = (int)(100.0 * d);
	d = (float)nd / 100.0;
	return d;
}
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, const CBlockIndex* pindex)
{
	...
	int nH = nPoWHeight / 1000;
	if (nH > 2861) {
		nSubsidy = 0.01 * COIN;
	}
	else {
		float f = computeFactor(nH);
		if (f < 0.01)
			f = 0.01;
		nSubsidy = f * COIN;
	}
...
static const int YEARLY_POW_BLOCK_COUNT = 315360;
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, const CBlockIndex* pindex)
{
	...
	if (nPoWHeight < YEARLY_POW_BLOCK_COUNT)
	{
		nSubsidy = 50 * nRewardCoinYear * nCoinAge / 365 / 3;
	}
	else if (nPoWHeight < 2 * YEARLY_POW_BLOCK_COUNT)
	{
		nSubsidy = 20 * nRewardCoinYear * nCoinAge / 365 / 3;
	}
	else if (nPoWHeight < 3 * YEARLY_POW_BLOCK_COUNT)
	{
		nSubsidy = 8 * nRewardCoinYear * nCoinAge / 365 / 3;
	}
	else
	{
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // Alexandrite: check proof-of-stake block signature
...
// Alexandrite: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Alexandrite", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Alexandrite: a rare cyber gem!";
        CTransaction txNew;
        txNew.nTime = 1485331053;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1485331053;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1948612;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
       ...
       if (true && (block.GetHash() != hashGenesisBlock)) {
        ...
        assert(block.hashMerkleRoot == uint256("0x2f244ec58dbf8939b800bb44b8022d5f5c239f4404b8687ff139be957eda7eec"));
